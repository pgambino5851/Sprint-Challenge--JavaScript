...
for(i = 0; i < universities.length; i++){
  smallestString = universities[i];
  for(j = 0; j < universities.length; j++){
    if((universities[j] > smallestString)){ //if new string comes BEFORE current smallest string
      tempUniv = smallestString;
      smallestString = universities[j];
      universities[i] = smallestString;
      universities[j] = tempUniv;
    }
  }
}
